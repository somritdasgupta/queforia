...
export function BrandKit() {
  ...
  return (
    <motion.div
      ...
      className="w-full backdrop-blur-xl bg-white/[0.02] rounded-2xl overflow-hidden border border-white/10 shadow-2xl relative"
    >
      {/* Glass effect background */}
      <div className="absolute inset-0 bg-gradient-to-br from-violet-500/5 via-transparent to-fuchsia-500/5" />

      {/* Tab Navigation */}
      <div className="relative z-10 flex p-1 gap-1 bg-black/20 backdrop-blur-md">
        {TABS.map((tab) => (
          <button
            key={tab.id}
            ...
            className={`
              ...
              ${
                activeTab === tab.id
                  ...
                  : "text-white/60 hover:bg-white/5 hover:border-white/10 border border-transparent"
              }
            `}
          >
            <tab.icon size={18} className="opacity-80" />
            <span className="font-prompt">{tab.label}</span>
          </button>
        ))}
      </div>
    ...
    <div className="space-y-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="backdrop-blur-xl bg-white/[0.02] rounded-2xl border border-white/10 p-6 space-y-6">
            <h3 className="text-xl font-bold font-prompt text-white bg-clip-text text-transparent bg-gradient-to-r from-white via-white/90 to-white/80">
              Logo Playground
            </h3>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-lg bg-white/5 border border-white/10 backdrop-blur-sm">
                <div className="space-y-4">
                  ...
                  {/* Modern Slider */}
                  <div className="relative h-7 flex items-center px-[2px]">
                    {/* Size markers */}
              ...
              >
                ...
                <div className="absolute inset-0 pointer-events-none">
                  {/* Vertical lines */}
                  {[0, 100, 200, 300, 400, 500].map((px) => (
                    <div
                      key={px}
                      className="absolute top-0 bottom-0 w-px bg-white/5"
                      style={{ left: `${(px / 580) * 100}%` }}
                    >
                      <span className="absolute top-2 -left-3 text-[10px] font-mono text-white/40">
                        {px}
                      </span>
                    </div>
                  ))}
                  ...
                  {[0, 100, 200].map((px) => (
                    <div
                      ...
                      style={{ top: `${(px / 200) * 100}%` }}
                    >
                      <span className="absolute left-2 -top-3 text-[10px] font-mono text-white/40">
                        {px}
                      </span>
                    </div>
                  ))}
              ...
              <div className="rounded-xl bg-white/5 border border-white/10 p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { name: "Dark Mode", bg: "#050a30", fg: "#ffffff" },
                    { name: "Light Mode", bg: "#f2efeb", fg: "#050a30" },
